    ...
    methods: {
        async userLogin() {
            try {
                ...
                // this.$auth.login({ data: this.login }).then(() => this.$toast.success('Logged In!'))
                // Устанавливаем токен в localStorage с помощью $auth.setToken
                this.$auth.setToken('local', response.access) 
